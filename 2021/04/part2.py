def play_bingo():
    with open("input.txt") as bingo_details:
        ...
        for move in moves:
            for board in range(board_count):
                if board not in boards or move not in boards[board]:
                    continue
                ...
                if finished:
                    print("Winning Board: ", board + 1)
                    print(f"Winning Move: {int(move)}, Sum Unmarked: {sum(unmarked)}")
                    print(f"Final Score: {int(move) * sum(unmarked)}\n")
                    boards.pop(board)
                    boards_row_col.pop(board)
                    finished = False
